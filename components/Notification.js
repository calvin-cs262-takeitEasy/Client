...
function Notification(props) {
  ...
  const bedtime_success_list = [`${props.name} went to bed on time!`];
  const study_success_list = [`${props.name} studied!`];
  const alarm_success_list = [`${props.name} woke up on time!`];
  ...
  if (props.Text == 'bedtime_fail') {
    ...
    text = `You shouldn't be seeing this. Invalid notification type: ${
      props.Text
    }. `;
  }
